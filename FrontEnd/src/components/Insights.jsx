...
function Insights() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        // For production, you might want to redirect to login page
        // window.location.href = "/";
      } finally {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE_URL}/home/expense?userId=${userId}`,
        { withCredentials: true }
      );
      setExpenses(res.data);
      setError("");
  ...
  const calculateMonthlyStats = () => {
    try {
      ...
      setError("Failed to calculate monthly statistics.");
    }
  };
  ...
  const generateInsights = async () => {
    ...
    try {
      ...
      // Fallback to rule-based insights
      setInsights(generateRuleBasedInsights());
      setError("Using basic insights due to analysis error.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (authLoading) {
    return (
      <div className="ml-4 flex justify-center items-center h-64">
  ...
  // Show error state for authentication
  if (error && !userId) {
    return (
      <div className="ml-4">
  ...
  return (
    <div className="ml-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          className="border rounded-md p-2 pl-4 pr-4 flex gap-2 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          onClick={refreshInsights}
      ...
      <div className="grid grid-cols-2 w-full rounded-md mt-6 bg-gray-100">
        <button
          value="Recommendations"
          className={`rounded-md py-2 px-1 md:px-2 m-1 text-sm md:text-base transition-colors ${
            buttonType === "Recommendations"
              ? "bg-white shadow-sm"
