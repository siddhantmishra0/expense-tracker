...
function Expenses() {
  ...
  const fetchBudget = async () => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE_URL}/home/budget?userId=${userId}`,
        { withCredentials: true }
      );
      setBudget(res.data);
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      setLoading(true);
      await axios.delete(`${API_BASE_URL}/home/expense/${id}`, {
        withCredentials: true,
      });
      setExpenses(expenses.filter((expense) => expense._id !== id));
      await fetchBudget(); // Refresh budget data after deletion
      setError("");
    } catch (error) {
      console.log("Error in deleting expense ", error);
  ...
  if (loading && !userId) {
    return (
      <div className="w-full ml-5 flex justify-center items-center h-64">
