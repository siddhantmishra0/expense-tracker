...
function Budget() {
  ...
  const budgetData = {
    ...
    datasets: [
      ...
      {
        label: "Spent",
        data: [budgets.Food*0.8, budgets.Transport*0.6, budgets.Bills*0.9, budgets.Shopping*0.9],
  ...
  return (
    <div className="w-full ml-5">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="border rounded-md p-4 md:p-6">
          ...
          <div className="flex flex-col mt-4 md:mt-6 gap-2">
            ...
            <select id="category" className="border rounded-md p-2 w-full" value={budgetType} onChange={(e)=> setBudgetType(e.target.value)}>
          ...
          <div className="flex flex-col mt-4 gap-2">
            ...
            <input id="budget" type="number" value={budgetAmount} className="border rounded-md p-2 w-full" onChange={(e)=>setBudgetAmount(e.target.value)}/>
          </div>
          {/* <div className="flex flex-col mt-4 gap-2">
          ...
          <div className="flex justify-end">
            <button className="border rounded-md bg-black text-white p-2 px-4 mt-4" onClick={handleAdd}>
              Add Budget
            </button>
          </div>
        </div>
        <div className="border rounded-md p-4 md:p-6">
          <div className="font-semibold text-xl md:text-2xl">Budget vs. Spending</div>
          <div className="text-gray-500 text-sm md:text-base">
      ...
      <div className="border rounded-md mt-4 p-4 md:p-6">
        ...
        <div className="text-gray-500 text-sm md:text-base">Monthly spending limit</div>
        <div className="flex flex-col sm:flex-row sm:justify-between mt-4 mb-4">
          <div className="flex flex-col mb-2 sm:mb-0">
            <div className="font-bold text-xl md:text-2xl">{`$${parseFloat(budgets.Overall) * 0.7}`}</div>
            <div className="text-gray-500 text-sm md:text-base">{`spent of $${parseFloat(budgets.Overall)}`}</div>
          </div>
      ...
      <div className="border rounded-md mt-4 p-4 md:p-6">
        <div className="font-semibold text-xl md:text-2xl">Category Budgets</div>
        <div className="text-gray-500 text-sm md:text-base">
          Track spending across different categories
        </div>
        <div className="mt-4">
          {[
            { name: "Food", budget: budgets.Food, spent: budgets.Food * 0.8, progress: 80, trending: "down" },
            { name: "Transport", budget: budgets.Transport, spent: budgets.Transport * 0.6, progress: 60, trending: "down" },
            { name: "Bills", budget: budgets.Bills, spent: budgets.Bills * 0.9, progress: 90, trending: "down" },
            { name: "Shopping", budget: budgets.Shopping, spent: budgets.Shopping * 0.9, progress: 140, trending: "up" },
          ].map((category, index) => (
            <div key={index} className="mb-4 pb-2 border-b last:border-b-0">
              <div className="flex flex-wrap justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className={category.trending === "up" ? "text-red-500" : "text-green-400"}>
                    {category.trending === "up" ? <TrendingUp size={16} /> : <TrendingDown size={16} />}
                  </div>
                </div>
              </div>
